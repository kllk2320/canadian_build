...
make -j4 -l install_root=${SYSROOT} install-bootstrap-headers=yes install-headers >>${LOGFILE} 2>&1 || { echo "Compiling libc-startfiles failed"; exit 1; }
...
make -j4 -l csu/subdir_lib >>${LOGFILE} 2>&1 || { echo "Installing libc-startfiles failed"; exit 1; }
